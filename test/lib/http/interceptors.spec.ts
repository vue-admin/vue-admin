...
function errorAdapter(status: number, data: unknown) {
  return async (config: InternalAxiosRequestConfig): Promise<AxiosResponse> => {
    const err = Object.assign(new Error('Request failed'), {
      ...
      isAxiosError: true,
      toJSON() { return { message: 'Request failed', code: 'ERR_BAD_REQUEST' } },
    })
    throw err
  }
}
...
describe('interceptors', () => {
  ...
  it('HTTP 200 + code=0 解包 data', async () => {
    instance.defaults.adapter = successAdapter({ code: 0, data: { id: 1 }, msg: 'ok' })
    const res = await instance.get('/x')
    expect(res.data).toEqual({ id: 1 })
  })

  it('HTTP 4xx 抛 HttpError 含 ProblemDetail', async () => {
    const problem = { type: 'about:blank', title: 'Bad Request', status: 400, detail: 'invalid' }
  ...
  it('silent 请求错误不触发全局 ElMessage', async () => {
    instance.defaults.adapter = errorAdapter(400, { type: 'x', title: 't', status: 400, detail: 'd' })
