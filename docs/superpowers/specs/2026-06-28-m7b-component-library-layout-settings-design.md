...
<template>
  <PageContainer title="用户管理">
    ...
    >
      ...
      <template #actions>
        <el-button type="primary" :icon="Plus" @click="openDrawer('add')">新增用户</el-button>
    ...
    <UserFormDrawer v-model="drawerVisible" :mode="drawerMode" :data="currentRow" @success="fetchList" />
  </PageContainer>
</template>
...
const columns = [
  ...
  { prop: 'actions', label: '操作', slot: 'actions', fixed: 'right', width: 150 }
]
...
export interface FormField {
  prop: string
  label: string
  type: 'input' | 'select' | 'radio' | 'checkbox' | 'switch' | 'date' | 'textarea' | 'number'
...
test('layout 设置 drawer 可开关 TagsView', async ({ page }) => {
  ...
  // 关闭 TagsView
  await page.locator('.setting-item', { hasText: '显示 TagsView' })
