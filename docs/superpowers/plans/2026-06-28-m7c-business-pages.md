...
it('view 模式 footer 仅显示关闭按钮', async () => {
  ...
  await flushPromises()
  const footerButtons = wrapper.findAll(
    '.el-drawer__footer .el-button, .el-drawer footer .el-button'
  )
  const footerText =
    wrapper.find('footer').text() + wrapper.find('.el-drawer__footer').text()
...
<template>
  ...
  >
    <div v-loading="loading" class="form-drawer-body">
      <el-form
        ref="formRef"
        :model="formData"
        :rules="rules"
...
import { ElMessage } from 'element-plus'
import { fetchRolePermissions, setRolePermissions } from '../../role/api'
import { fetchAllPermissions, type PermissionInfo } from '../../permission/api'
...
const handleSave = async () => {
  ...
  try {
    ...
    const permissionIds = keys.filter(
      (k) =>
        !MODULE_LABELS[k] &&
        !Object.prototype.hasOwnProperty.call(MODULE_LABELS, k)
    )
...
const {
  ...
  fetch: (params) =>
    fetchPermissionList(params as unknown as PermissionSearchRequest),
...
<template>
  <!-- 搜索和工具栏区域 -->
  <el-card shadow="never" class="search-card"></el-card>
</template>
```
...
export default [
  ...
  },
  {
    ...
    response: ({
      ...
      body: { draggingId: string; targetId: string; position: string }
    }) => {
...
const drawerTitle = computed(() => {
  if (props.mode === 'add')
    return props.parent ? `新增子菜单 - ${props.parent.name}` : '新增顶级菜单'
...
const treeSelectData = computed(() => {
  const transform = (nodes: MenuInfo[]): any[] =>
    nodes.map((n) => ({
      ...
      label: n.name,
      children:
        n.children && n.children.length > 0 ? transform(n.children) : undefined
    }))
...
const fields = computed<FormField[]>(() => [
  ...
  { prop: 'path', label: '路由路径', type: 'input', span: 12 },
  {
    ...
    type: 'input',
    span: 12,
    placeholder: '如 dashboard/views/Home'
  },
  {
    prop: 'icon',
    label: '图标',
  ...
  <PageContainer title="菜单管理">
    <template #header>
      <el-button
        type="primary"
        :icon="Plus"
        @click="openDrawer('add', null, null)"
      >
        新增顶级菜单
      </el-button>
          ...
          <div class="tree-node">
            ...
            <span class="tree-node__actions">
              <el-button
                ...
                type="primary"
                size="small"
                @click.stop="openDrawer('add', data, data)"
              >
                新增子菜单
              </el-button>
              <el-button
                ...
                type="primary"
                size="small"
                @click.stop="openDrawer('edit', data, data)"
              >
                编辑
              </el-button>
              <el-button
...
test.describe.serial('业务页面闭环', () => {
  test('admin 列表渲染 + 查看 drawer 打开', async ({ page }) => {
    ...
    await viewBtn.click()
    await expect(
      page.getByRole('heading', { name: '查看管理员' })
    ).toBeVisible()
  })
  ...
  test('menu 树渲染 + 新增顶级菜单 drawer', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: '新增顶级菜单' })
    ).toBeVisible()
  })
})
```
